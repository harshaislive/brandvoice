...
class BeforestBrandVoice:
    ...
    def load_settings(self):
        """Load settings from Supabase database only"""
        try:
            ...
            if self.supabase:
                try:
                    ...
                    if result.data:
                        ...
                        raise Exception("No settings data found in database")
                except Exception as db_error:
                    logger.error(f"Failed to load settings from database: {str(db_error)}")
                    raise Exception(f"Database settings load failed: {str(db_error)}")
            else:
                logger.error("Supabase not configured - cannot load settings")
                raise Exception("Supabase not configured")
                
        except Exception as e:
            logger.error(f"Settings loading failed: {str(e)}")
            # Don't create default settings, let the error propagate
            self.settings = None
...
def get_settings():
    ...
    if brand_voice.settings is None:
        return jsonify({
            'success': False, 
            'error': 'Settings could not be loaded from database. Please check Supabase connection.'
        }), 500
